...
import type { ReclaimInAppCapacitorSdkPlugin } from './definitions';
...
export abstract class ReclaimVerificationPlatform {
...
export class ReclaimVerificationPlatformImpl extends ReclaimVerificationPlatform {
    ...
    override async setOverrides({
        ...
        if (providerCallback) {
            ...
            let providerRequestSubscription = await ReclaimInAppCapacitorSdk.addListener('onProviderInformationRequest', async (event: NativeReclaimInappModuleTypes.ProviderInformationRequest) => {
                try {
                    let result = await providerCallback(event);
                    ReclaimInAppCapacitorSdk.replyWithProviderInformation({ replyId: event.replyId, providerInformation: result });
                } catch (error) {
                    console.error(error);
                    ReclaimInAppCapacitorSdk.replyWithProviderInformation({ replyId: event.replyId, providerInformation: "" });
                }
            });
            const cancel = async () => {
        ...
        if (onLogsListener) {
            ...
            this.previousLogSubscription = await ReclaimInAppCapacitorSdk.addListener('onLogs', async (arg: { value: string }) => {
